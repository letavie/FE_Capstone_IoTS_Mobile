...
//           searchKeyword: '',
//         };
//         // const labResponse = await fetchLabMemberPagination(comboId as string, labParams);
...
//           return (
//             <View className="bg-white p-6 rounded-lg shadow-md mb-4">
//  <Text className="text-2xl font-semibold text-blue-600 mb-6">Related Combos</Text>
//  <FlatList
...
//         {/* Fixed Bottom Bar */}
//         <View
//   className="bg-white p-4 border-t border-gray-200 shadow-lg absolute bottom-0 left-0 right-0"
// >
//   <View className="flex-row items-center justify-between gap-4">
//     <Text className="text-xl font-bold text-red-600 flex-1">
//       {((selectedCombo?.price || 0) * numCart).toLocaleString('vi-VN')} VND
//     </Text>
...
//         onPress={() => onChangeQuantity(numCart + 1)}
//         className="p-3"
//         disabled={selectedCombo?.quantity ? numCart >= selectedCombo.quantity : false}
//       >
//         <Text
//           className={`text-lg font-semibold ${
              ...
              labs.map((item) => (
                ...
                >
                  ...
                  <View className="flex-1">
                    <Text className="text-gray-800 font-semibold">{item.title}</Text>
                    <Text className="text-gray-600 text-sm">Summary: {item.summary}</Text>
                    <Text className="text-gray-600 text-sm">Price: {item.price.toLocaleString('vi-VN')} VND</Text>
                    <Text className="text-gray-700 text-sm mt-1">
                      Store: <Text className="text-blue-600 font-medium">{item.storeName}</Text>
                    </Text>
                  </View>
      ...
      <View className="bg-white p-4 border-t border-gray-200 shadow-lg absolute bottom-0 left-0 right-0">
        <View className="flex-row items-center justify-between gap-4">
          ...
          <View className="flex-row items-center bg-gray-100 rounded-full">
            ...
            >
              <Text
                className={`text-lg font-semibold ${numCart <= 1 ? 'text-gray-400' : 'text-gray-800'}`}
              >
                −
              </Text>
            </TouchableOpacity>
          ...
          <TouchableOpacity
            onPress={handleAddToCart}
            disabled={isAdding || (selectedCombo?.quantity ?? 0) === 0}
            className={`flex-1 py-3 rounded-lg ${isAdding || (selectedCombo?.quantity ?? 0) === 0 ? 'bg-gray-300' : 'bg-blue-600'}`}
          >
            <Text
              className={`text-center text-base font-semibold ${
                isAdding || (selectedCombo?.quantity ?? 0) === 0 ? 'text-gray-500' : 'text-white'
              }`}
            >
              {isAdding ? 'Adding...' : 'Add to Cart'}
            </Text>
