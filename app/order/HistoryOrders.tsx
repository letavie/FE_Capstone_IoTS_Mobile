...
// import { fetchHistoryOrder, changeFeedbackStatus, getTrackingGhtk, changeCancelledStatus, changeCancelledCashPayment } from '@/services/slices/historyOrderApi';
...
//   const [selectedSellerGroup, setSelectedSellerGroup] = useState<SellerGroup | null>(null);
...
//     orderItemId: null,
//   });
//   const [historyOrders, setHistoryOrders] = useState<HistoryOrdersResponse>({ totalCount: 0, dataHistoryOrder: [] });
//   const [loading, setLoading] = useState<boolean>(false);

//   const fetchOrders = async (keyword: string = searchKeyword) => {
//     setLoading(true);
//     try {
...
//         // Ensure sellerRole is always provided by setting a default value if missing
//         dataHistoryOrder: response.data.data.map((order: Order) => ({
//           ...order,
//           orderDetailsGrouped: order.orderDetailsGrouped.map((group: SellerGroup) => ({
//             ...group,
...
//         type: 'error',
//         text1: 'Error',
//         text2: error instanceof Error ? error.message : 'Failed to fetch orders.',
//       });
//     } finally {
//       setLoading(false);
//     }
...
//   const handleFeedbackClick = (group: SellerGroup) => {
...
//   const handleWarrantyRequestClick = (orderItemId: number) => {
...
//   const handleCancelClick = (orderId: number, sellerId: number) => {
//     setCancelOrderInfo({
//       orderId,
//       sellerId,
//       visible: true,
...
//     const { contactNumber, accountName, accountNumber, bankName } = cancelOrderInfo.formData;
...
//         type: 'error',
//         text1: 'Error',
//         text2: error instanceof Error ? error.message : 'Failed to cancel order.',
//       });
//     }
//   };

//   const handleCancelCashPayment = (orderId: number) => {
//     Alert.alert(
//       'Cancel Order',
...
//   const handleCancelFormChange = (name: keyof CancelOrderInfo['formData'], value: string) => {
//     setCancelOrderInfo({
//       ...cancelOrderInfo,
//       formData: {
...
//   const handleCancelFormBlur = (field: keyof CancelOrderInfo['touched']) => {
//     setCancelOrderInfo((prev) => ({
//       ...prev,
//       touched: {
//         ...prev.touched,
...
//   const handleTrackClick = async (trackingId: string) => {
//     try {
...
//         type: 'error',
//         text1: 'Error',
//         text2: error instanceof Error ? error.message : 'Failed to fetch tracking info.',
//       });
//     }
//   };
...
//   const handleChangeToFeedback = (orderId: number, sellerId: number) => {
//     Alert.alert(
...
//                 type: 'error',
//                 text1: 'Error',
//                 text2: error instanceof Error ? error.message : 'Failed to update order status.',
//               });
//             }
//           },
//         },
...
//         onCancel={() => setCancelOrderInfo({ ...cancelOrderInfo, visible: false })}
...
import { View, Text, TextInput, FlatList, ActivityIndicator, Alert, SafeAreaView, StyleSheet } from 'react-native';
import { useRouter } from 'expo-router';
import Toast, { ToastShowParams } from 'react-native-toast-message';
import { fetchHistoryOrder, changeFeedbackStatus, getTrackingGhtk, changeCancelledStatus, changeCancelledCashPayment } from '@/services/slices/historyOrderApi';
import { useThemeColors } from '@/hooks/useThemeColors';
...
export default function HistoryOrders() {
  ...
  const fetchOrders = useCallback(async (keyword: string = searchKeyword) => {
    setLoading(true);
    try {
      const response = await fetchHistoryOrder({
        pageIndex: 1,
  ...
  const handleCancelCashPayment = useCallback((orderId: number) => {
    Alert.alert(
      'Cancel Order',
      'Are you sure you want to cancel this order?',
      [
  ...
  const handleCancelFormChange = useCallback((name: keyof CancelOrderInfo['formData'], value: string) => {
    setCancelOrderInfo((prev) => ({
      ...prev,
      formData: {
        ...prev.formData,
  ...
  const handleCancelFormBlur = useCallback((field: keyof CancelOrderInfo['touched']) => {
    setCancelOrderInfo((prev) => ({
      ...prev,
      touched: {
        ...prev.touched,
  ...
  const handleChangeToFeedback = useCallback((orderId: number, sellerId: number) => {
    Alert.alert(
      'Confirm Received Order',
      'Are you sure you have received this order?',
      [
