...
// export default function OrderTabs({ statusFilter, onChange, totalCount, children }: OrderTabsProps) {
...
//             <TouchableOpacity
//               key={key}
//               className={`flex-row items-center px-3 py-2 mr-2 rounded-lg ${key === (statusFilter || '0') ? 'bg-primary' : 'bg-blue-200'}`}
//               onPress={() => onChange(key)}
//             >
//               <Text className="mr-2">{config.icon}</Text>
//               <Text
//                 className="font-medium"
//                 style={{ color: key === (statusFilter || '0') ? colors.primaryForeground : colors.foreground }}
//               >
//                 {config.tabName}
//               </Text>
...
export default function OrderTabs({
  ...
  return (
    <View>
      ...
      {/* Content */}
      {children && <View>{children}</View>} {/* Chỉ render children nếu có */}
    </View>
  );
}
